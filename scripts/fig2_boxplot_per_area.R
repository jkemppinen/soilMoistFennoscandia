...
# plot
dev.off()
pdf(file="D:/ARTIKKELIT/ARTIKKELI8/analysis/results/fig2/fig2_boxplot_per_area.pdf")

your_sm %>% 
  filter(year(date) %in% c(2020)) %>%
  filter(month(date) %in% c(5, 6, 7, 8, 9)) %>%
  ggplot(aes(x=(date), y=moist_mean, fill = area, color = area)) +
  
  geom_boxplot (aes(group=date),
                notch=TRUE, notchwidth = 0.1,
                width=0.5, position=position_dodge(1),
                outlier.size = 0.1, outlier.shape=20,
                lwd=0.25)  +
