...
p_sd_all_smooth = spatial_data  %>% 
  ...
  scale_fill_manual(values = rev(your_palette(7))) +
  scale_color_manual(values = rev(your_palette(7))) +
  ylab ("Spatial heterogeneity\n(Standard deviation VWC%)") +
  xlab ("Time") +
  ylim (0, 21) +
  scale_x_date(date_breaks = "1 month", date_labels =  "%d.%m.") +
  theme_classic() +
...
dev.off()
pdf(file="fig/fig_app.pdf", width = 7.48, height = 5)

wrap_plots(A = p_mean_all_smooth,
           B = p_sd_all_smooth,
           C = p_names, design = layout)

dev.off()

spatial_data %>% group_by(area) %>% 
  summarise(mean(sm_sd, na.rm = T))
